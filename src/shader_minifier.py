...
def process_files(args):
    ...
    for filename in args.files:
        ...
        # Cut it into several pieces.print(your_path))
        if len(minified_source_code) > max_string_len:
            for i in range(int(len(minified_source_code) / max_string_len) + 1):
                f = open("minified/minified_{}.{}".format(basename, i), "w")
                f.write('R"(' + minified_source_code[
                                i * max_string_len:min((i + 1) * max_string_len, len(minified_source_code))] + ')"')
                f.close()
        else:
            f = open("minified/minified_" + basename, "w")
            f.write('R"(' + minified_source_code + ')"')
            f.close()
